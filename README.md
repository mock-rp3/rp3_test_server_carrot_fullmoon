...
### 202108-24 2차 피드백
+ 상품 데이터 GET Query 수정 (inner join을 사용하여 테이블이 조회 안 되는 상황 해결)
+ 네이버 sens 활용하여 문자 인증할 수 있음
+ 끌올 초로 변환해서 클라이언트로 보내면 클라이언트에서 처리할 수 있음
+ HTTPS로 API 요청할 시 오류는 Nginx 설정 파일 멘토에게 공유할 것
+ 유저 인덱스는 UUID 알고리즘으로 보여줄 수 있음
+ category 존재하지 않는 것 입력했을 때의 validation 처리할 것
+ 더미데이터 충분하게 만들 것
+ 메인화면 지역별로 볼 수 있도록 할 것
+ 채팅은 POST로 글 쓰듯이 구현할 수 있음
+ 이미지 주소랑 확장자 validation 처리할 것
...
+ 채팅방 조회 API (20%)
+ 프록시 패스 적용
+ 중고 상품 쿼리 수정 (삭제 판매글 조회 문제)
+ 중고 상품 디테일 페이지 조회수 추가

### 2021-08-26 진행상황
+ 게시자 판매 상품, 판매내역 조회 URI 및 response 수정
+ 회원가입과 로그인 API 통합 (30%)
+ 채팅방 전체 조회 API 개발
+ 채팅방 입장 조회 API 개발
...
### 2021-08-27 진행상황
+ 로그인 및 회원가입 API 통합
+ 등록 및 끌올 일시 시간 포맷 업데이트 (-분 전, -일 전으로 보이게끔 서버에서 넘겨줌)
+ 상품 정렬 시간순 변경
+ 관심 목록 조회 API 개발
+ 상품 키워드 등록 API 개발
+ 동네생활 카테고리 조회 API 개발
...
+ 해결 과정 
1. ```/etc/nginx/sites-available```에 서버 블록 만들지 않고 따로 파일을 만들어 설정 (해결)
...
13. 인스턴스 새로 만든 뒤 certbot을 클래식 버전으로 설치함 (해결)
